...
var Notifier = /** @class */ (function () {
    ...
    Notifier.prototype.sendToRoom = function (type, roomId, data) {
        var roomConnectedClients = instances_1.roomClients.get(roomId);
        if (!roomConnectedClients)
            return;
        var roomData = {
            type: type,
            data: __assign(__assign({}, data), { roomId: roomId }),
        };
        var message = JSON.stringify(roomData);
        for (var _i = 0, _a = Array.from(roomConnectedClients); _i < _a.length; _i++) {
            var client = _a[_i];
    ...
    Notifier.prototype.sendToUser = function (type, userId, roomId, data) {
        var _this = this;
        var client = instances_1.clients.get(userId);
        if (!client)
            return;
        var clientsData = {
            type: type,
    ...
    Notifier.prototype.broadcastToClients = function (type, data) {
        ...
        instances_1.clients.forEach(function (client) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                ...
                if (!type || !roomId || !isPublic && !userId)
                    return [2 /*return*/];
                if ([
                    MessageType_1.MessageType.ADD_ROOM,
                    MessageType_1.MessageType.REMOVE_ROOM,
                    MessageType_1.MessageType.UPDATE_ROOM
                ].includes(type)) {
                    this.broadcastToClients(type, data);
                }
                ;
                if (type == MessageType_1.MessageType.UPDATE_ROOM_STATE) {
